...
Так как мы имеем дело с белым ящиком, то фактически можем видеть все ветвления. Мы можем построить модель логики, содержащейся в белом ящике, чтобы использовать её для генерации тестовых данных. В случае, если тестируемый код написан на Scala, можно, например, использовать [scalameta](https://scalameta.org/). Далее будем предполагать, что тестируемый код реализован с использованием ограниченного подмножества языка, либо на другом языке, который изначально ограничен.
