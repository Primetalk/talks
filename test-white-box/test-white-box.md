...
Если изначальная реализация не поддерживала некоторую функциональность, то наши тесты ничего такого не заметят.
